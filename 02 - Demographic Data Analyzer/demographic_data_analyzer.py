...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round(num_min_workers.salary.value_counts(normalize=True)['>50K'],1) * 100

    # What country has the highest percentage of people that earn >50K?
    workingDF = pd.Series(  [1-df[df['native-country'] == country].salary.value_counts(normalize=True)['<=50K'] for country in df['native-country'].drop_duplicates() ],
                            index = df['native-country'].drop_duplicates())
    highest_earning_country = workingDF.idxmax()
    highest_earning_country_percentage = round(workingDF.max() * 100, 1)
